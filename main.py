                ...
                if db.session is not None:
                    ...
                    if 'btnSiDeal' in request.form:
                        if request.form['btnSiDeal'] == 'Si':
                            ...
                            direcciones = db.session.query(LugarUsuario).filter_by(usuarioId=usuario.idUsuario,
                                                                                   activa=1, principal=1).first()

                            if direcciones is not None:
                                if len(transaccion) == 0:
                                    mensaje = '{usuario} aceptó el trato para el juego {juego}, por favor envíale una respuesta.'.format(
                                        usuario=usuario.nombres, juego=obVideojuego.nombre)

                                    qya = funcCrearUpdateQA(idtrx, mensaje, usuario.idUsuario, 'aceptar')

                                    habilitar = ''
                                elif len(transaccion) == 1:
                                    mensaje = 'Muy bien, se cerró el trato por el juego {juego}, nuestro equipo de logistica coordinará pronto para recogerlo/enviarlo..'.format(
                                        juego=obVideojuego.nombre)
